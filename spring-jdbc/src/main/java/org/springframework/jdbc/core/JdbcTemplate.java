...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
    ...
    //-------------------------------------------------------------------------
    // 核心方法：传入不同的PreparedStatementCallback执行不同的操作：update或query
    @Override
    @Nullable
    public <T> T execute(PreparedStatementCreator psc, PreparedStatementCallback<T> action)
        ...
        try {
            ...
            // 应用用户对PreparedStatement设置一些属性
            applyStatementSettings(ps);
            // 通过PreparedStatement执行sql操作，获取执行结果
            T result = action.doInPreparedStatement(ps); // 调用回调函数
            handleWarnings(ps);
            return result;
        } catch (SQLException ex) {
            // Release Connection early, to avoid potential connection pool deadlock
    ...
    public int update(String sql, @Nullable PreparedStatementSetter pss) throws DataAccessException {
        ...
        SimplePreparedStatementCreator simplePreparedStatementCreator = new SimplePreparedStatementCreator(sql);
        return update(simplePreparedStatementCreator, pss);
    }

    @Override
    public int update(String sql, Object[] args, int[] argTypes) throws DataAccessException {
        // 对参数和参数类型进行封装
        PreparedStatementSetter preparedStatementSetter = newArgTypePreparedStatementSetter(args, argTypes);
    ...
    protected void applyStatementSettings(Statement stmt) throws SQLException {
        ...
        if (fetchSize != -1) {
            ...
            // fetchSize的含义是：当调用rs.next()方法时会一次性从服务器上获取多少条数据，这样下次调用rs.next()时，会从内存中读取，
            // 但是fetchSize比较大的时候，很占用内存。
        }
