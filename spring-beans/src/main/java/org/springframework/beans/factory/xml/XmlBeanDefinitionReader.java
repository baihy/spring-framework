...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
     ...
     *                                      <p>
     *                                      把resource对象解析成指定的成员变量
     */
    @Override
    public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
        // EncodedResource对资源文件进行编码
    ...
    public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
        ...
        try {
            ...
            try {
                // 把io里面的InputStream对象封装成org.xml.sax里面的InputStream对象。
    ...
    protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
        ...
        try {
            //把resource对象转换成document对象
            Document doc = doLoadDocument(inputSource, resource);
            // 通过document对象，进行注册beanDefinition对象
    ...
    protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
        // 获取 xml文件的验证模式，保证了xml配置文件正确性，验证模式有：DTD和XSD两种模式
        int validationModeForResource = getValidationModeForResource(resource);
        // 这个对象携带了sax对xml文件进行验证的声明。
        EntityResolver entityResolver = getEntityResolver();
        // 这个documentLoader委托给了DefaultDocumentLoader这个对象
    ...
    public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
        ...
        // 根据配置文件转换的document对象，进行注册BeanDefinition
